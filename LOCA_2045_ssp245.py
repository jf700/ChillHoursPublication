...
# === Settings ===
target_years = [2043, 2044, 2045, 2046, 2047]
Tc_celsius = 7.2
processed_data_filename = "2045_245_ensemble_chill_hours.nc"
shapefile_path = r"C:\Users\Josh\Documents\ChillResearch\Fruit_Nuts\Fruits_Nuts.shp"

# === File structure with two time periods ===
model_files = {
    "MIROC6": {
        "tasmin": [
            "tasmin.MIROC6.ssp245.r1i1p1f1.2015-2044.LOCA_16thdeg_v20220413.west.nc",
            "tasmin.MIROC6.ssp245.r1i1p1f1.2045-2074.LOCA_16thdeg_v20220413.west.nc"
        ],
        "tasmax": [
            "tasmax.MIROC6.ssp245.r1i1p1f1.2015-2044.LOCA_16thdeg_v20220413.west.nc",
            "tasmax.MIROC6.ssp245.r1i1p1f1.2045-2074.LOCA_16thdeg_v20220413.west.nc"
        ]
    },
    "CanESM5": {
        "tasmin": [
            "tasmin.CanESM5.ssp245.r1i1p1f1.2015-2044.LOCA_16thdeg_v20220413.west.nc",
            "tasmin.CanESM5.ssp245.r1i1p1f1.2045-2074.LOCA_16thdeg_v20220413.west.nc"
        ],
        "tasmax": [
            "tasmax.CanESM5.ssp245.r1i1p1f1.2015-2044.LOCA_16thdeg_v20220413.west.nc",
            "tasmax.CanESM5.ssp245.r1i1p1f1.2045-2074.LOCA_16thdeg_v20220413.west.nc"
        ]
    },
    "HadGEM3-GC31-LL": {
        "tasmin": [
            "tasmin.HadGEM3-GC31-LL.ssp245.r1i1p1f3.2015-2044.LOCA_16thdeg_v20220413.west.nc",
            "tasmin.HadGEM3-GC31-LL.ssp245.r1i1p1f3.2045-2074.LOCA_16thdeg_v20220413.west.nc"
        ],
        "tasmax": [
            "tasmax.HadGEM3-GC31-LL.ssp245.r1i1p1f3.2015-2044.LOCA_16thdeg_v20220413.west.nc",
            "tasmax.HadGEM3-GC31-LL.ssp245.r1i1p1f3.2045-2074.LOCA_16thdeg_v20220413.west.nc"
        ]
    },
    "CNRM-CM6-1": {
        "tasmin": [
            "tasmin.CNRM-CM6-1.ssp245.r1i1p1f2.2015-2044.LOCA_16thdeg_v20220413.west.nc",
            "tasmin.CNRM-CM6-1.ssp245.r1i1p1f2.2045-2074.LOCA_16thdeg_v20220413.west.nc"
        ],
        "tasmax": [
            "tasmax.CNRM-CM6-1.ssp245.r1i1p1f2.2015-2044.LOCA_16thdeg_v20220413.west.nc",
            "tasmax.CNRM-CM6-1.ssp245.r1i1p1f2.2045-2074.LOCA_16thdeg_v20220413.west.nc"
        ]
    }
}
...
def compute_corrected_chill_hours_weinberger(tasmin, tasmax, smoothing_sigma=5):
    ...
    for i in range(tasmin_c.shape[0]): # Loop through each day
        ...
        for lat_idx in range(num_lats): # Loop through each latitude grid cell
            for lon_idx in range(num_lons): # Loop through each longitude grid cell
                ...
                current_daily_ch_count = 0
                for hour_temp in estimated_hourly_temps_c:
                    if hour_temp > 0 and hour_temp < Tc_celsius:
                        current_daily_ch_count += 1
    ...
    return chill_hours * smoothed_correction_da
...
if os.path.exists(processed_data_filename):
    ...
    for model_name, paths in model_files.items():
        ...
        try:
            # Use open_mfdataset to open and combine the list of files automatically
            ds_min_full = xr.open_mfdataset(paths["tasmin"], combine='by_coords')["tasmin"]
            ds_max_full = xr.open_mfdataset(paths["tasmax"], combine='by_coords')["tasmax"]
            chill_stack = []
            for year in target_years:
                ...
                tasmin_season, tasmax_season = ds_min_full.sel(time=time_mask), ds_max_full.sel(time=time_mask)
...
ax.set_title("Ensemble Chill Hours (Nov-Feb, 2043–2047) for Fruit and Nuts Area - SSP2-4.5", fontsize=12)
...
print(f"Scenario: SSP2-4.5, 2045")
